...
/**
 * POST /api/community/posts/:id/save — idempotent toggle.
...
export async function POST(_req, { params }) {
  ...
  try {
    ...
    if (existing) {
      ...
      await prisma.communityPostSave.create({ data: { postId: id, userId: user.id } });
    }

    const total = await prisma.communityPostSave.count({ where: { postId: id } });

    return Response.json({ active: !existing, total });
  } catch (error) {
    console.error("[community] failed to toggle save", error);
